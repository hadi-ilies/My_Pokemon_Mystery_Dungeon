...
bool read_entity_from_fd(int fd, entity_t *entity)
{
	if (read(fd, &entity->level, sizeof(size_t)) != sizeof(size_t))
		return (entity->error = ERR_READ, false);
	...
	for (size_t i = 0; i < 4; i++)
		if (read(fd, &entity->capacity_num[i], sizeof(size_t))
		!= sizeof(size_t))
			return (entity->error = ERR_READ, false);
	for (size_t i = 0; i < 4; i++)
...
bool read_entity_from_fd_2(int fd, entity_t *entity, size_t *len)
{
...
entity_t entity_create_from_fd(int fd)
{
	...
	if (!read_entity_from_fd(fd, &entity)
	|| !read_entity_from_fd_2(fd, &entity, &len))
		return (entity);
	...
	entity.anime_tab =
		anime_tab_create_from_file(entity.anime_tab_file_name);
